...
class Interpreter {
    fun interpret(ast: ASTInterface): Map<String, Variable> {
        ...
        when (root) {
            TokenType.DECLARATION -> {
                addVariableToMap(variableMap, getVariableName(ast), getVariableType(ast))
            }
            ...
            TokenType.ASSIGNATION -> {
                if (!checkIfAssignationAndDeclaration(ast.getChildren()[0].getToken().getType())) {
                    ...
                    addVariableToMap(variableMap, variableName, searchForVariableType(variableName, variableMap), variableValue)
                } else {
                    val variableName = getVariableName(ast.getChildren()[0])
                    val variableType = getVariableType(ast.getChildren()[0])
                    val variableValue = recursiveSearch(ast.getChildren()[1], variableType)
                    addVariableToMap(variableMap, variableName, variableType, variableValue)
                }
            }
    ...
    private fun searchForVariableType(
        variableName: String,
        variableMap: HashMap<String, Variable>,
    ): TokenType {
    ...
    private fun recursiveSearch(
        ...
        return when (val root = ast.getToken().getType()) {
            ...
            TokenType.INTEGER -> ast.getToken().getValue()
            // Esto se podría encargar de mirarlo el SCA, en caso de que no vengan operators o string o ints
            TokenType.OPERATOR -> {
    ...
    private fun getVariableOrValueType(
        itemToken: Token,
        variableMap: Map<String, Variable>,
    ): TokenType {
    ...
    private fun getVariableOrValue(
        itemToken: Token,
        variableMap: Map<String, Variable>,
    ): String {
        if (checkIfIsIdentifier(itemToken)) {
