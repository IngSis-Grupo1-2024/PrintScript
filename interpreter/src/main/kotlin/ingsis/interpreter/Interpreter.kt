...
object PrintScriptInterpreter {
    ...
    fun createInterpreter(version: String): Interpreter {
        return when(version) {
            "VERSION_1" -> Interpreter(listOf(DeclarationInterpreter(), AssignationInterpreter(), CompoundAssignationInterpreter(), PrintLineInterpreter()))
            else -> Interpreter(listOf(DeclarationInterpreter(), AssignationInterpreter(), CompoundAssignationInterpreter(), PrintLineInterpreter()))
        }
    }
}
