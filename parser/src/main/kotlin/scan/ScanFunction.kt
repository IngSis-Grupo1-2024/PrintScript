...
class ScanFunction: ScanStatement {
    ...
    private fun valueCanHandle(tokens: List<Token>) =
        scanValue.canHandle(tokens.subList(functionTypes.size, tokens.size-1))

    private fun hasLastParenthesis(tokenTypes: List<TokenType>) = tokenTypes.last() != TokenType.PARENTHESIS

    private fun hasFunctionTypes(tokenTypes: List<TokenType>) =
        tokenTypes.subList(0, functionTypes.size) == functionTypes
