...
//        }
    ...
    private fun isCallingMethod(tokens: List<Token>): Boolean {
        ...
        // sum(a + b)
        val tokenTypes = tokens.map { it.getType() }
        if (tokenTypes.subList(0, functionTypes.size) == functionTypes) {
            if (tokenTypes.last() != TokenType.PARENTHESIS) {
                throw ParserError(
                    tokens.last().getPosition().startLine.toString() + ":error: ')' expected " + tokens.last().getPosition(),
                    tokens.last(),
                )
            }
            return checkValue(tokens, functionTypes.size, tokens.size - 1)
        }
        return false
    }
